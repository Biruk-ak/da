...
const mainServices = [
  ...
  {
    icon: Cloud,
    title: "SaaS Design & Hosting",
    tagline: "Scalable Infrastructure for Growth",
    description: "A cloud-based model where providers deliver applications over the internet, typically via subscriptions.",
    features: ["Subscription Model", "Automatic Updates", "Multi-tenancy"],
...
export default function ServicesPage() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground transition-colors duration-500">
      ...
      <section ref={heroRef} className="relative min-h-[80vh] md:min-h-[85vh] bg-background text-foreground flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-12 lg:px-20 py-24 md:py-32 pt-32 md:pt-40">
          <div className="max-w-4xl">
            <div className="services-hero-reveal inline-flex items-center p-1 px-3 md:px-4 mb-6 md:mb-8 bg-foreground/5 rounded-full border border-foreground/10">
              <span className="w-2 h-2 bg-lime rounded-full mr-2 md:mr-3 animate-pulse" />
              <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-foreground/60">Our Services</span>
            </div>

            <h1 className="services-hero-reveal text-[2rem] sm:text-[2.5rem] md:text-[3rem] lg:text-[5rem] xl:text-[6rem] font-[900] leading-[0.95] tracking-tighter uppercase mb-6 md:mb-8 text-foreground">
              Full-Service
              <br />
            ...
            <p className="services-hero-reveal text-foreground/50 text-base md:text-lg lg:text-xl leading-relaxed mb-8 md:mb-10 max-w-2xl">
            ...
            <div className="services-hero-reveal flex flex-col sm:flex-row gap-3 md:gap-4">
              <Link
                href="/contact"
                className="inline-flex items-center justify-center gap-2 md:gap-3 bg-lime text-black px-6 md:px-8 py-3.5 md:py-4 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] hover:bg-foreground hover:text-background transition-all duration-500 group"
              >
                Start a Project
              ...
              <Link
                href="/projects"
                className="inline-flex items-center justify-center gap-2 md:gap-3 border border-foreground/20 text-foreground px-6 md:px-8 py-3.5 md:py-4 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] hover:bg-foreground/10 transition-all duration-500"
              >
                View Our Work
              </Link>
            </div>
        ...
        <div className="absolute right-10 top-1/2 -translate-y-1/2 hidden xl:flex flex-col gap-4">
          {[Code2, Shield, Cloud, Cpu].map((Icon, i) => (
            <div key={i} className="services-hero-reveal w-14 h-14 bg-foreground/5 border border-foreground/10 rounded-2xl flex items-center justify-center hover:bg-lime hover:border-lime transition-all group">
              <Icon className="w-6 h-6 text-foreground/40 group-hover:text-black transition-colors" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-16 md:py-24 lg:py-32 px-4 sm:px-6 md:px-12 lg:px-20 bg-background">
        <div className="max-w-7xl mx-auto">
          <ScrollReveal className="text-center mb-12 md:mb-20">
            ...
            <p className="text-foreground/40 text-base md:text-lg max-w-2xl mx-auto px-4">
      ...
      <section className="py-16 md:py-24 lg:py-32 px-4 sm:px-6 md:px-12 lg:px-20 bg-foreground/[0.03] border-t border-foreground/5">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
            {additionalServices.map((service, index) => (
              <ScrollReveal key={index} delay={index * 0.05}>
                <div className="bg-secondary/50 rounded-xl md:rounded-2xl p-4 md:p-8 h-full border border-foreground/10 hover:border-lime/50 transition-all duration-300 group">
      ...
      {/* Industries Section */}
      {/* <section className="py-16 md:py-24 lg:py-32 px-4 sm:px-6 md:px-12 lg:px-20 bg-background text-foreground border-y border-foreground/5"> */}
      {/* <div className="max-w-7xl mx-auto"> */}
      {/* <div className="grid lg:grid-cols-2 gap-10 md:gap-16 items-center"> */}
      {/* <ScrollReveal>
              <div className="inline-flex items-center p-1 px-3 md:px-4 mb-4 md:mb-6 bg-foreground/5 rounded-full border border-foreground/10">
                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-foreground/60">Industries We Serve</span>
              </div>
              <h2 className="text-[2rem] sm:text-[2.5rem] md:text-[3.5rem] font-[900] leading-[0.95] tracking-tighter uppercase mb-4 md:mb-6 text-foreground">
                Expertise Across
                <br />
      ...
      {/* <ScrollReveal delay={0.2}>
              <div className="flex flex-wrap gap-2 md:gap-3">
                {industries.map((industry, index) => (
                  <span
                    key={index}
                    className="px-3 md:px-5 py-2 md:py-3 bg-foreground/5 border border-foreground/10 rounded-full text-xs md:text-sm font-bold hover:bg-lime hover:text-black hover:border-lime transition-all cursor-default text-foreground"
                  >
                    {industry}
                  </span>
                ))}
            ...
            {[
              ...
              <ScrollReveal key={index} delay={index * 0.1}>
                <div className="relative">
                  <span className="text-[3rem] md:text-[6rem] font-black text-foreground/5 leading-none">{item.step}</span>
      ...
      {/* CTA Section */}
      <section className="py-16 md:py-24 lg:py-32 px-4 sm:px-6 md:px-12 lg:px-20 bg-lime border-t border-foreground/10">
        <div className="max-w-5xl mx-auto text-center">
          <ScrollReveal>
            <h2 className="text-[2rem] sm:text-[2.5rem] md:text-[3rem] lg:text-[4rem] font-[900] leading-[0.95] tracking-tighter uppercase mb-4 md:mb-6 text-black">
              Ready to Transform
              <br />
            ...
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 md:gap-3 bg-black text-lime px-8 md:px-10 py-4 md:py-5 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] hover:bg-white hover:text-black transition-all duration-500 group"
            >
              Get Started Today
              <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </Link>
