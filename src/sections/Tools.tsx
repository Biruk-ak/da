...
const features = [
  {
    title: "Innovation",
    description: "We push the boundaries of digital innovation, helping clients see beyond traditional approaches.",
    icon: Lightbulb,
  ...
  {
    title: "Excellence",
    description: "Driven by a passionate and skilled team to deliver high quality, relevant results.",
    icon: Target,
    bgColor: "bg-foreground text-background",
    descColor: "text-background/70",
  },
  {
    title: "Integrity",
    description: "We focus on building lasting, trust-based partnerships with every client.",
    icon: Rocket,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mb-4 md:mb-6 items-stretch">
          {features.map((feature, index) => (
            <ScrollReveal
              key={index}
              delay={index * 0.1}
              className={`${feature.bgColor} rounded-2xl md:rounded-3xl p-5 md:p-6 border border-foreground/5 shadow-sm flex flex-col`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          ...
          <ScrollReveal className="bg-foreground text-background rounded-2xl md:rounded-3xl p-8 md:p-12 flex flex-col justify-between">
            ...
            <button className="bg-lime text-black px-8 py-4 md:py-5 rounded-full font-bold uppercase tracking-[0.2em] text-[10px] md:text-xs flex items-center justify-center gap-3 group hover:bg-background hover:text-foreground transition-colors w-full sm:w-max mt-auto">
          ...
          <div className="bg-lime rounded-2xl md:rounded-3xl p-6 md:p-8">
            ...
            <div className="md:hidden flex flex-wrap gap-2">
              {techStack.map((tech, i) => (
                <span
                  key={i}
                  className={`px-3 py-1.5 rounded-full text-xs font-medium ${["Next.js", "React", "Tailwind CSS", "TypeScript"].includes(tech)
                    ? "bg-black text-lime"
