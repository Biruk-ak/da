    ...
    <section className="py-16 md:py-32 px-4 sm:px-6 md:px-12 lg:px-20 bg-background overflow-hidden">
      <div className="max-w-[1400px] mx-auto">
        ...
        <ScrollReveal className="mb-12 md:mb-24 text-center relative">
          <div className="inline-flex items-center justify-center p-1 px-3 md:px-4 mb-6 md:mb-8 bg-foreground/5 rounded-full border border-foreground/5">
            <span className="w-2 h-2 bg-primary rounded-full mr-2 md:mr-3 animate-pulse" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-12 items-stretch">
          ...
          <div className="lg:col-span-7">
            <ScrollReveal delay={0.2} className="h-full">
              <form onSubmit={handleSubmit} className="bg-foreground/5 p-6 md:p-8 lg:p-12 rounded-[2rem] md:rounded-[3.5rem] border border-foreground/5 h-full flex flex-col justify-center">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-4 md:mb-6">
                  <div className="relative group">
                    <User className="absolute left-4 md:left-6 top-1/2 -translate-y-1/2 h-4 w-4 md:h-5 md:w-5 text-gray-400 group-focus-within:text-primary transition-colors" />
                    <Input
                      placeholder="FULL NAME"
                      className="pl-12 md:pl-16 py-6 md:py-8 rounded-[1.25rem] md:rounded-[1.5rem] bg-background border-foreground/10 shadow-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-bold text-[10px] md:text-xs tracking-wider md:tracking-widest uppercase text-foreground"
                      value={formData.fullName}
                      onChange={(e) => setFormData({ ...formData, fullName: e.target.value })}
                    />
                  </div>
                  <div className="relative group">
                    <Mail className="absolute left-4 md:left-6 top-1/2 -translate-y-1/2 h-4 w-4 md:h-5 md:w-5 text-gray-400 group-focus-within:text-primary transition-colors" />
                    <Input
                      type="email"
                      placeholder="EMAIL ADDRESS"
                      className="pl-12 md:pl-16 py-6 md:py-8 rounded-[1.25rem] md:rounded-[1.5rem] bg-background border-black/10 dark:border-white/10 shadow-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-bold text-[10px] md:text-xs tracking-wider md:tracking-widest uppercase text-foreground dark:text-white"
                ...
                <div className="mb-4 md:mb-6">
                  ...
                  >
                    <SelectTrigger className="py-6 md:py-8 px-4 md:px-6 rounded-[1.25rem] md:rounded-[1.5rem] bg-background border-foreground/10 shadow-sm font-bold text-[10px] md:text-xs tracking-wider md:tracking-widest uppercase text-foreground">
                ...
                <Button
                  ...
                  className="w-full bg-lime text-black hover:bg-foreground hover:text-background rounded-full py-7 md:py-10 text-[10px] md:text-[12px] font-[1000] uppercase tracking-[0.4em] md:tracking-[0.5em] transition-all duration-500 shadow-xl shadow-lime/20 border-none disabled:opacity-70"
                >
                  {isSubmitting ? "Sending Request..." : "LET’S CREATE TOGETHER"}
                </Button>
